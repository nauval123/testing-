...
declare module '@ioc:Adonis/Core/Validator' {
    ...
    export type AsyncValidation<T extends any = unknown> = {
        compile(type: NodeType, subtype: NodeSubType, options?: any, rulesTree?: {
            [key: string]: any;
        }): ParsedRule<T>;
     ...
     */
    export type EnumReturnValue<Options extends AllowedEnumOptions> = Options extends SchemaRef<infer R> ? R extends readonly unknown[] ? R[number] : unknown : Options extends readonly unknown[] ? Options[number] : never;
    /**
     * Conditionally finds the return value for the EnumSet options
     */
    export type EnumSetReturnValue<Options extends AllowedEnumOptions> = Options extends SchemaRef<infer R> ? R extends readonly unknown[] ? R[number][] : unknown : Options extends readonly unknown[] ? Options[number][] : never;
    /**
    ...
    export interface EnumType {
        ...
        optional<Options extends AllowedEnumOptions>(options: Options, rules?: Rule[]): {
            t?: EnumReturnValue<Options>;
            getTree(): SchemaLiteral;
        };
    }
    ...
    export interface Schema {
        ...
        array: ArrayType;
        file: FileType;
        refs: <T extends Object>(refs: T) => {
            [K in keyof T]: SchemaRef<T[K]>;
        };
    ...
    export type ValidatorConfig = {
        ...
        reporter?: () => Promise<ErrorReporterConstructorContract> | Promise<{
            default: ErrorReporterConstructorContract;
        }>;
    };
    /**
     ...
     */
    export type RequestValidatorNode<T extends ParsedTypedSchema<TypedSchema>> = (new (ctx: HttpContextContract) => WithOptional<ValidatorNode<T>, 'data'>) | WithOptional<ValidatorNode<T>, 'data'>;
    /**
     * Shape of the function that validates the compiler output
     */
    export type ValidateFn = <T extends ParsedTypedSchema<TypedSchema>>(validator: ValidatorNode<T>) => Promise<T['props']>;
    /**
    ...
    export interface Rules {
         ...
         * expecations are met
         */
        requiredWhen(field: string, operator: 'in' | 'notIn', comparisonValues: any[] | SchemaRef<any[]>): Rule;
        requiredWhen(field: string, operator: '>' | '<' | '>=' | '<=', comparisonValue: number | SchemaRef<number>): Rule;
        requiredWhen(field: string, operator: 'in' | 'notIn' | '=' | '!=' | '>' | '<' | '>=' | '<=', comparisonValues: any | SchemaRef<any>): Rule;
        /**
        ...
        mobile(options?: {
            strict?: boolean;
            locales?: validatorJs.MobilePhoneLocale[];
        }): Rule;
        /**
